...
class Saver:
    ...
    def _generate_save_path(self, file_path):
        if 'Mid-Level Perceptual Features' in file_path.split('/'):
            file_path = file_path.split('.')[0]
            file_name = os.path.split(file_path)[1]
            save_path = os.path.join(self.feature_save_dir, file_name + ".npy")
        elif 'Emotify' in file_path.split('/'):
            file_path = file_path.split('/')
            
            print(file_path)
            # Extract the filename without extension
            audio_number = int(file_path[-1].split('.')[0])
            
            # Rock + 100
            if file_path[-2] == "rock":
                audio_number += 100
            # Electronic + 200
            elif file_path[-2] == "electronic":
                audio_number += 200
            # Pop + 300
            elif file_path[-2] == "pop":
                audio_number += 300
            save_path = os.path.join(self.feature_save_dir,  str(audio_number) + ".npy")
...
if __name__ == "__main__":
    ...
    # SPECTROGRAM_SAVE_DIR = "Mid-Level Perceptual Features/spectrograms/"
    # FILES_DIR = "Mid-Level Perceptual Features/audio/"
    
    SPECTROGRAM_SAVE_DIR = "Emotify/spectrograms/"
    FILES_DIR = "Emotify"
